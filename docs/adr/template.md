# <Title>

## Status
<!-- Proposed | Accepted | Implemented | Deprecated -->

## Context
<!--
What is the problem or situation that motivated this decision?
Include any constraints, requirements, or background that someone
reading this cold would need to understand.
-->

## Decision
<!--
What was decided? Be specific.
If applicable, include a brief description of how it works.
-->

## Consequences
<!--
What are the outcomes of this decision — both positive and negative?
-->

**Benefits:**
-

**Trade-offs:**
-

## Alternatives Considered
<!--
What other options were evaluated and why were they not chosen?
-->